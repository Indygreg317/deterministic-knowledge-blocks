...
This index helps readers locate the artifacts that support verification, replay, receipt inspection, evaluator behavior, governance boundary review, schema inspection, validation coverage, validation reports, governance-record concept review, and audit-package inspection.
...
| Schema definitions | Define structured artifact expectations. | `schema/knowledge-block.schema.json`; `schema/deterministic-outcome.schema.json`; `schema/execution-receipt.schema.json`; `schema/verification-report.schema.json`; `schema/governance-record.schema.json`; `schema/governance-boundary-map.schema.json`; `schema/audit-package-manifest.schema.json`; `schema/artifact-validation-manifest.schema.json`; `schema/schema-validation-report.schema.json`; `schema/decision-contract.schema.json`; `schema/quantum-output.schema.json`; `schema/evaluator-policy.schema.json`; `docs/schema-catalog.md` | Present | Schemas make artifacts reviewable and machine-checkable. |
...
| Schema validation reports | Record manifest checks, artifact case results, expected-invalid behavior, summary counts, and failures. | `schema/schema-validation-report.schema.json`; `docs/schema-validation-reports.md`; `validation/reports/schema-validation-report.json`; `audit-packages/bell-state-minimal/audit-package-manifest.json`; generated by `reference-implementation/python/validate_artifact_schemas.py --save-report` | Present | Includes a persisted example report, CI-generated temporary report artifact, and audit-package reference. |
...
| Governance record concept | Documents governance records as declaration artifacts and distinguishes declaration from verification. | `docs/governance-records.md` | Present | Concept documentation only; does not prove governance declarations. |
| Governance record examples | Declare scope, authority, policy, or continuity assumptions. | `use-cases/quantum/examples/bell-state-governance-record.json` | Present | Minimal Bell-state governance record example; validation manifest coverage is planned for a future PR. |
...
| Audit package examples | Bundle evidence into reviewer-facing inspection paths. | `schema/audit-package-manifest.schema.json`; `docs/audit-packages.md`; `audit-packages/bell-state-minimal/README.md`; `audit-packages/bell-state-minimal/audit-package-manifest.json`; `validation/reports/schema-validation-report.json` | Present | Minimal package indexes the current Bell-state evidence chain, including schema validation report evidence. |
...
- a schema validation report that records manifest checks and artifact case results
- a validation coverage index that maps schema checks to artifacts and expected outcomes
- a governance record concept document that distinguishes declarations from proof
- a governance record example that makes scope, authority context, continuity assumptions, and linked evidence reviewable
- a governance boundary map that separates declarations from verified claims
- an audit package manifest that indexes schema validation reports, receipts, verification reports, and boundary artifacts for reviewer inspection
...
- validation manifest coverage for governance record examples
